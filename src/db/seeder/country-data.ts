/**
 * Canonical country reference data — the single source of truth shared by
 * `seed-countries.ts` (which upserts it) and `dedupe-countries.ts` (which
 * repairs rows that drifted from it).
 */
export type CountrySeed = { name: string; code: string; callCode: string };

export const countryData: CountrySeed[] = [
